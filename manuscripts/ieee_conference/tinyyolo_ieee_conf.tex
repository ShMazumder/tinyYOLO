...
\end{equation}
where $k$ is the kernel size, $\text{GAP}$ represents global average pooling, and $\sigma$ represents the sigmoid function. The total backbone parameters are 80.4K (standard) and 72.6K (quantized). The near-identical GFLOPs between variants (0.25 vs. 0.24) reflects that SE and ECA contribute negligibly to total FLOPs at the small channel widths used (80, 160 channels), where convolution operations dominate compute.

Table~\ref{tab:arch_spec} provides the complete backbone stage configuration required for reproduction.

\begin{table}[h]
\centering
\caption{TinyYOLO Backbone Architecture Specification}
\label{tab:arch_spec}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lccccc}
\toprule
Stage & Operator & Output & Ch & Blocks ($d_i$) & Stride \\
\midrule
Stem & Conv3$\times$3-BN-Act & 208$\times$208 & 16 & 1 & 2 \\
S1 & GhostBN & 104$\times$104 & 24 & 1 & 2 \\
S2 & GhostBN & 52$\times$52 & 40 & 2 & 2 \\
S3 & GhostBN + Attn & 26$\times$26 & 80 & 3 & 2 \\
S4 & GhostBN + Attn & 13$\times$13 & 160 & 2 & 2 \\
\bottomrule
\end{tabular}
\vskip 2pt
\begin{flushleft}
\scriptsize Input: 416$\times$416. Stride-2 downsampling is performed by the first GhostBottleneck in each stage. Attention: Std=LightSpatial(S3)+SE(S4); Quant=ECA(S3)+ECA(S4).
\end{flushleft}
\end{table}

\subsection{Neck: LitePAN Multi-Scale Feature Fusion}
The neck fuses features across scales using a bidirectional pathway: top-down (FPN) for semantic enrichment of shallow features, and bottom-up (PAN) for spatial enrichment of deep features. LitePAN shares the bidirectional topology of CSP-PAN (PicoDet~\cite{yu2021picodet}) and Ghost-PAN (NanoDet~\cite{lyu2020nanodet}), but replaces all intermediate processing with pure depthwise separable convolutions for minimal parameter overhead:
\begin{align}
...
\textbf{Observer and calibration protocol.} For PTQ, we use \texttt{MinMaxObserver} for both weights (\texttt{per\_tensor\_symmetric}, \texttt{qint8}) and activations (\texttt{per\_tensor\_affine}, \texttt{quint8}), calibrated on 500 batches ($\sim$8,000 images) from the training set with augmentation disabled. For QAT, we use PyTorch's default QAT configuration (\texttt{get\_default\_qat\_qconfig('qnnpack')}), which uses \texttt{FakeQuantize} with \texttt{MovingAverageMinMaxObserver}. QAT fine-tuning runs for 10 epochs at $\text{lr}=10^{-4}$ with AdamW and cosine annealing. Observer statistics are frozen in the last 25\% of QAT epochs, and batch normalization statistics are frozen concurrently to stabilize final quantization parameters.
...
Additionally, unlike YOLO-Fastest which dedicates its entire budget to single-task detection, TinyYOLO's shared backbone successfully supports instance segmentation and pose estimation, showcasing its extreme capacity-efficiency.
...
On Jetson Nano, peak RAM drops from 48 MB (FP32) to 18 MB (INT8), representing a 2.6$\times$ reduction. The model file size drops to 0.22 MB under INT8. Energy per frame is computed as $E = P_{\text{TDP}} \times t_{\text{inference}}$, yielding upper-bound estimates of $5\text{W} \times 28.3\text{ms} = 141.5$ mJ/frame on Jetson Nano and $5\text{W} \times 67.4\text{ms} = 337$ mJ/frame on Raspberry Pi 4. These represent TDP-based upper bounds; actual power draw during INT8 inference is expected to be lower due to reduced memory bandwidth and ALU utilization. During sustained inference, the thermal profile stabilizes at 58$^\circ$C (Jetson Nano) and 62$^\circ$C (Raspberry Pi 4) without thermal throttling.
...
Replacing standard convolutions with Ghost convolutions reduces parameters by 46\% and FLOPs by 50\%, with a minor mAP reduction of 2.4\% (from 39.8\% to 37.4\%). As a convenience metric, GhostConvs achieve 170 mAP/M-params vs.\ standard convs' 97 mAP/M-params; we note that this ratio is not a standard benchmark metric but provides intuitive context for parameter utilization at this scale.
...
TAL assignment improves mAP by 7.8\% over single-cell assignment and reduces training epochs to reach 30\% mAP by 41\% (from 82 to 48 epochs). We adopt TAL hyperparameters from TOOD~\cite{jocher2023yolov8}: $k{=}10$, $\alpha{=}0.5$, $\beta{=}6.0$. These values were not specifically tuned for our sub-0.3M regime; task-specific hyperparameter optimization remains future work. The dedicated objectness head, following YOLOX's~\cite{ge2021yolox} decoupled design, increases mAP by 2.6\% and reduces false positive detections by 62\% compared to the max-class-logit proxy --- the explicit objectness signal provides the model with a direct mechanism to distinguish foreground from background without relying on indirect class score magnitude. Finally, mosaic augmentation disabled in the last 10\% of epochs contributes +4.3\% mAP.

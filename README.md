...
| **R1.3** | 2026-05-25 | Overhauled the Average Precision (AP) mathematical calculations and class-averaged mAP evaluation protocols in `tinyYOLO/utils/metrics.py`. Resolved the **AP Linear Interpolation Inflation Bug** by replacing standard linear interpolation (`np.interp`) with a mathematically correct vectorized **101-point step-function interpolation** (COCO style), eliminating low-recall AP overestimations of up to 500% and boundary sentinel underestimation bugs. Corrected the class-averaging logic to skip categories with **zero ground-truth annotations** ($N_{gt} = 0$) in validation/test splits from the denominator of mAP, fully conforming with official `pycocotools` / COCOeval standard protocols. |
